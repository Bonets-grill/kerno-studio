...
body{font-family:'Nunito Sans',sans-serif;background:var(--content-bg);color:var(--text);display:flex;height:100vh;overflow:hidden;font-size:14px;line-height:1.5}
...
/* ── TOP NAVBAR ── */
.topbar{height:56px;background:var(--sidebar-bg);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;padding:0 28px;z-index:99}
...
<nav class="bottom-nav">
  ...
  <button onclick="showPage('citas')"><span class="bnav-icon">◎</span>Citas</button>
  <button onclick="showPage('pacientes')"><span class="bnav-icon">☰</span>Pacientes</button>
  <button onclick="showPage('historial')"><span class="bnav-icon">▤</span>Historial</button>
  <button onclick="showPage('analytics')"><span class="bnav-icon">◈</span>Analytics</button>
  <button onclick="showPage('configuracion')"><span class="bnav-icon">⚙</span>Config</button>
</nav>
